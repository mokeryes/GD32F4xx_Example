...
/**
 *  -------------------------------------------------------------------
 * | SMPLRT_DIV | Bit7 | Bit6 | Bit 5 | Bit4 | Bit3 | Bit2 | Bit1 | Bit0 |
 *  -------------------------------------------------------------------
 * |    0x19    |                 SMPLRT_DIV[7:0]                        |
 *  -------------------------------------------------------------------
 * 陀螺仪数据输出频率设置 
...
/** 
 *  -------------------------------------------------------------------
 * |  CONFIG  | Bit7 | Bit6 | Bit 5 | Bit4 | Bit3 | Bit2 | Bit1 | Bit0 |
 *  -------------------------------------------------------------------
...
/**
 *  ---------------------------------------------------------------------
 * | GYRO_CONFIG | Bit7 | Bit6 | Bit5 | Bit4 | Bit3 | Bit2 | Bit1 | Bit0 |
 *  ------------------------------------------------------------------
...
/**
 *  ----------------------------------------------------------------------
 * | ACCEL_CONFIG| Bit7 | Bit6 | Bit 5 | Bit4 | Bit3 | Bit2 | Bit1 | Bit0 |
 *  -------------------------------------------------------------------
 * |     0x1C    |XA_ST |XY_ST |XZ_ST  |AFS_SEL[1:0] |   ACCEL_HPF[2:0]   |
 *  -----------------------------------------------------------------------
...
/**
 *  -------------------------------------------------------------------
 * |  FF_THR  | Bit7 | Bit6 | Bit 5 | Bit4 | Bit3 | Bit2 | Bit1 | Bit0 |
 *  -------------------------------------------------------------------
 * |   0x1D   |                      FF_THR[7:0]                       |
 *  -------------------------------------------------------------------
 * 自由落体运动检测临界值设置
 * 加速度传感器三轴的绝对值与寄存器中自由落体运动触发阈值(用户可通过此寄存器阈值进行设置)作比较，每一次自由落体运动阈值被触发都将使FF_DUR自增，
 * 当FF_DUR的累加值达到用户设定值时，即判定设备正在进行自由落体运动，FF_DUR到达阈值后将不会再自增；当FF_THR的值小于用户设定值时，
 * FF_DUR将会自减至0。
 ...
 * |   0x1F   |                      MOT_THR[7:0]                      |
 *  -------------------------------------------------------------------
 * 与自由落体运动检测相似，当加速度传感器任意轴的绝对值大于运动检测阈值时，会使MOT_DUR自增，累加到MOT_DUR用户设定的阈值时，则判定设备出在运动状态。
 */
 ...
 *  -------------------------------------------------------------------
 * 需要用到高通滤波器
 * 零点运动阈值，每当零点运动到达阈值，则使ZRMOT_DUR自增，与自由落体运动检测和运动检测不同，当设备零点运动被判定后，不再继续判定，直到自由落体运动或者
 * 运动检测判定。
 * 可通过读取MOT_DETECT_STATUS_REG寄存器来得到运动检测值和零点运动检测值。
 */
#define ZRMOT_THR_REG           0x21
